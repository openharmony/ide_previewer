...
namespace {
    ...
    void ChangeFlagHandler([[maybe_unused]] void *data)
    {
        g_flag = true;
    }

    TEST(AsyncWorkManagerTest, ExecAllAsyncWorkTest)
    {
        AsyncWorkManager::GetInstance().AppendAsyncWork(ChangeFlagHandler, nullptr);
    ...
    TEST(AsyncWorkManagerTest, ClearAllAsyncWorkTest)
    {
        AsyncWorkManager::GetInstance().AppendAsyncWork(ChangeFlagHandler, nullptr);
        EXPECT_EQ(AsyncWorkManager::GetInstance().workList.size(), 1); // 1 element in list
